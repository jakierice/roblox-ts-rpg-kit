...
export const map: <A, B>(
  f: (a: A) => B
) => <E>(fa: State<E, A>) => State<E, B> = (f) => (fa) => (s1) => {
  const [a, s2] = fa(s1)
  return [f(a), s2]
}
...
export const ap: <E, A>(
  fa: State<E, A>
) => <B>(fab: State<E, (a: A) => B>) => State<E, B> = (fa) => (fab) => (s1) => {
  const [f, s2] = fab(s1)
  const [a, s3] = fa(s2)
  return [f(a), s3]
}
...
export const chain: <E, A, B>(
  f: (a: A) => State<E, B>
) => (ma: State<E, A>) => State<E, B> = (f) => (ma) => (s1) => {
  const [a, s2] = ma(s1)
  return f(a)(s2)
}
...
export const bindTo = <N extends string>(
  name: N
): (<S, A>(fa: State<S, A>) => State<S, { [K in N]: A }>) => map(bindTo_(name))
...
export const traverseArrayWithIndex: <A, S, B>(
...
) => (arr: ReadonlyArray<A>) => State<S, ReadonlyArray<B>> = (f) => (arr) => (
  s
) => {
  let lastState = s
  ...
  for (let i = 0; i < arr.size(); i++) {
    ...
    values.push(newValue) // eslint-disable-line
    lastState = newState
  }
...
export const traverseArray: <A, S, B>(
  f: (a: A) => State<S, B>
) => (arr: ReadonlyArray<A>) => State<S, ReadonlyArray<B>> = (f) =>
