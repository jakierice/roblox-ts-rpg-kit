...
function curried(f: Function, n: number, acc: ReadonlyArray<unknown>) {
  return function (x: unknown) {
    // const combined = Array(acc.size() + 1)
    const combined = [...acc]
    for (let i = 0; i < acc.size(); i++) {
      combined[i] = acc[i]
    }
    combined[acc.size()] = x
    return n === 0 ? f(...combined) : curried(f, n - 1, combined) // eslint-disable-line
  }
}
...
function getTupleConstructor(len: number): (a: unknown) => any {
  ...
  // does NOT have property of len: number
  if (Object.keys(tupleConstructors).includes(len) === false) {
