...
function compare(x: number | string, y: number | string): Ordering {
  if (typeIs(x, "number") && typeIs(y, "number")) {
    return x < y ? -1 : x > y ? 1 : 0
  }

  if (typeIs(x, "string") && typeIs(y, "string")) {
    return x < y ? -1 : x > y ? 1 : 0
  }
  return 0
}
...
export const ordBoolean: Ord<boolean> = {
  equals: strictEqual,
  compare: (x, y) => (x === true && y === false ? 1 : 0),
}
...
export function fromCompare<A>(compare: (x: A, y: A) => Ordering): Ord<A> {
  const optimizedCompare = (x: A, y: A): Ordering =>
    x === y ? 0 : compare(x, y)
  return {
...
export function getSemigroup<A = never>(): Semigroup<Ord<A>> {
  return {
    concat: (x, y) =>
      fromCompare((a, b) =>
        monoidOrdering.concat(x.compare(a, b), y.compare(a, b))
      ),
  }
}
 ...
 * @since 2.0.0
 */
export function getTupleOrd<T extends ReadonlyArray<Ord<unknown>>>(
  ...ords: T
...
export const contramap: <A, B>(f: (b: B) => A) => (fa: Ord<A>) => Ord<B> = (
  f
) => (fa) => fromCompare((x, y) => fa.compare(f(x), f(y)))
